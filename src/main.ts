...
const renderSkills = () => {
    return portfolioData.skills.map(skill => `
        <div class="mb-6 animate-fade-in-up">
            <h4 class="text-xs font-bold text-slate-400 border-b border-slate-200 pb-1 mb-3 uppercase tracking-widest">${skill.category}</h4>
            <div class="flex flex-wrap gap-2">
                ${skill.items.map(item => `<span class="skill-badge">${item}</span>`).join('')}
            </div>
        </div>
    `).join('');
};
...
const renderExp = () => {
    return portfolioData.experience.map(exp => `
        <div class="timeline-line animate-fade-in-up">
            ...
            <h3 class="text-xl font-bold font-sans text-slate-800">${exp.role}</h3>
...
const renderCerts = () => {
    return portfolioData.certifications.map(cert => `
        <div class="p-4 bg-slate-50 rounded-xl border border-slate-100 hover:border-primary-200 transition-all hover:bg-white animate-fade-in-up">
            <h4 class="font-bold text-slate-800 text-sm mb-1">${cert.name}</h4>
            <p class="text-slate-500 text-xs">${cert.description}</p>
        </div>
    `).join('');
};
        ...
        <div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div class="space-y-6 order-2 lg:order-1 animate-fade-in-up">
                ...
                <h1 class="text-5xl md:text-7xl font-black text-slate-900 leading-tight">
                    Hi, I'm <span class="gradient-text">${portfolioData.profile.fullName}</span>
                </h1>
            ...
            <div class="relative order-1 lg:order-2 flex justify-center animate-fade-in">
                ...
                <div class="relative p-2 bg-white rounded-3xl shadow-2xl rotate-6 hover:rotate-0 hover:scale-105 transition-all duration-500 border border-slate-100 cursor-pointer">
                    <img src="${portfolioData.profile.photo}" alt="Profile" class="w-64 h-80 object-cover rounded-2xl shadow-inner">
                </div>
            </div>
        </div>
    </section>
                ...
                <div id="contact" class="card bg-slate-900 text-white shadow-xl">
                    ...
                    <ul class="space-y-4">
                        <li class="flex items-center gap-4 group cursor-pointer">
                            ...
                            <span class="text-slate-300 font-medium group-hover:text-white transition-colors uppercase tracking-tight text-sm">${portfolioData.profile.location}</span>
                        </li>
                        <li class="flex items-center gap-4 group cursor-pointer">
                            <div class="w-10 h-10 bg-slate-800 rounded-lg flex items-center justify-center group-hover:bg-primary-600 transition-colors">
                                <span class="text-xl">📧</span>
                            </div>
                            <span class="text-slate-300 font-medium group-hover:text-white transition-colors lowercase tracking-tight text-sm">${portfolioData.profile.email}</span>
                        </li>
                        <li class="flex items-center gap-4 group cursor-pointer">
                            <div class="w-10 h-10 bg-slate-800 rounded-lg flex items-center justify-center group-hover:bg-primary-600 transition-colors">
                                <span class="text-xl">📞</span>
                            </div>
                            <span class="text-slate-300 font-medium group-hover:text-white transition-colors tracking-tight text-sm">${portfolioData.profile.phone}</span>
                        </li>
                        <li class="flex items-center gap-4 group cursor-pointer">
                            <a href="${portfolioData.profile.linkedin}" target="_blank" class="flex items-center gap-4 w-full">
                                <div class="w-10 h-10 bg-slate-800 rounded-lg flex items-center justify-center group-hover:bg-primary-600 transition-colors">
                                    <span class="text-xl">🔗</span>
                                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <div class="card hover:border-primary-100">
                            <span class="text-primary-600 font-bold text-xs uppercase tracking-widest mb-1 block">${portfolioData.education.degree}</span>
                            <h3 class="text-lg font-bold text-slate-800">${portfolioData.education.institution}</h3>
                        </div>
                    </div>
